...
def random_walk(d, states, transition_matrix, start_state="（回到）首頁", steps=20, verbose=True, serial="UNKNOWN"):
    ...
    for i in range(steps):
        if verbose:
            print(f"[{serial}] Step {i+1}: {current_state}")
            sys.stdout.flush()
            ...
            match current_state:
                ...
                case "評論":
                    ...
                    if current_action_result:
                        current_action_result = post_comment_like_human(d)
                case "查看發文者主頁":
                ...
                case _:
                    print(f"[{serial}] 未知狀態：{current_state}")
                    sys.stdout.flush()

        except Exception as e:
            print(f"[{serial}] 執行 {current_state} 發生錯誤：{e}")
            sys.stdout.flush()


        if current_action_result is False:
            print(f"[{serial}] 動作「{current_state}」執行失敗，重新抽選狀態")
            sys.stdout.flush()
