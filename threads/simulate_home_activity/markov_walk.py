    ...
    for i in range(steps):
        ...
        try:
            match current_state:
                ...
                case "查看貼文細節":
                    action_result = click.click(d, resource_id="feed_post_text")

                case "貼文細節捲動":
                    action_result = swipe.swipe(d)

                case "愛心貼文":
                    action_result = click.click(d, resource_id="feed_post_ufi_like_button")

                case "評論貼文":
                    action_result = click.click(d, resource_id="feed_post_ufi_reply_button")
                    if action_result:
                        BASE_DIR = Path(__file__).resolve().parent
                        comment_path = BASE_DIR / "comments.txt"
                        comments = parse_comment.load_comments_from_txt(comment_path)
                        action_result = post_comment.post_comment(d, comments=comments, input_box_id="permalink_inline_composer", post_btn_id="permalink_inline_composer_post_button")
                case "（回到）首頁":
                    if prev_state:
